USE [SMDv2]
GO
/****** Object:  StoredProcedure [dbo].[getAdsCampaignByCampaignIdtblAnalytic]    Script Date: 11/25/2016 4:38:22 PM ******/
SET ANSI_NULLS ON
GO
...
DECLARE @dateFrom DATE = getdate()-30, @Ctype int = 1;
	...
	END
		Select 1 ordr, 'Total impressions' label, count(acr.CampaignID) 'All time' ,
		...
		Select 3 ordr, 'Click-thru' label, count(acr.CampaignID) 'All time' , 
			(Select count(acr.CampaignID) from AdCampaignResponse acr
		...
		union
		Select 4 ordr, 'Surveys answered' label, count(acr.CampaignID) 'All time' , 
			(Select count(acr.CampaignID) from AdCampaignResponse acr
		...
		union
		Select 5 ordr, 'Average ad click' label, isnull(ROUND((sum(t.DebitAmount)/count(t.DebitAmount))/100, 2), 0) 'All time' , 
			(Select isnull(ROUND((sum(t.DebitAmount)/count(t.DebitAmount))/100,2), 0) from [Transaction] t
...
--EXEC [getAdsCampaignByCampaignIdtblAnalytic] 20159
