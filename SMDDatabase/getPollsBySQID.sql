GO
/****** Object:  StoredProcedure [dbo].[getPollsBySQID]    Script Date: 10/24/2016 9:56:57 PM ******/
SET ANSI_NULLS ON
GO
...
IF @DateRange = 2
	BEGIN
	 Select top 1 @dateFrom= ResoponseDateTime from SurveyQuestionResponse where SQID = @SQID
	END
	...
	insert into @open
	Select  t.Granual, count(case when sqr.ResponseType = 1 then sqr.SQID else null end) OpenStats , min(t.date) ordr
	from SurveyQuestionResponse sqr 
	right outer join   @T1 t on t.date = CONVERT(date, sqr.ResoponseDateTime) and sqr.SQID = @SQID
	group by t.Granual

		insert into @stats
		Select  t.Granual, count(case when sqr.ResponseType = 3 then sqr.SQID else null end) AnsStats, count(case when sqr.ResponseType = 4 then sqr.SQID else null end) SkipStats
		from SurveyQuestionResponse sqr 
		right outer join   @T1 t on t.date = CONVERT(date, sqr.ResoponseDateTime) and sqr.SQID = @SQID
		group by t.Granual
