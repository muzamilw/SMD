...
IF @DateRange = 2
	...
	IF @status = 1 -- answered
		BEGIN
		insert into @stats
		Select  t.Granual, count(case when sqr.ResponseType = 3 then sqr.SQID else null end) Stats
		from SurveyQuestionResponse sqr 
		right outer join   @T1 t on t.date = CONVERT(date, sqr.ResoponseDateTime) and sqr.SQID = @SQID
		group by t.Granual
		END
	IF @status = 2 -- skipped
		BEGIN
		insert into @stats
		Select  t.Granual, count(case when sqr.ResponseType = 4 then sqr.SQID else null end) Stats
		from SurveyQuestionResponse sqr 
		right outer join   @T1 t on t.date = CONVERT(date, sqr.ResoponseDateTime) and sqr.SQID = @SQID
		group by t.Granual
		END
