      ...
      , methods = {
        init: function (opt) {
          this.each(function () {
              ...
              $spanLeft = $('<span></span>').addClass("labelLeft").append(options.label.enabled === undefined ? "ON" : options.label.enabled);
              $spanRight = $('<span></span>').addClass("labelRight").append(options.label.disabled === undefined ? "OFF " : options.label.disabled);
              ...
              $div.append($spanLeft);
              $div.append($('<label><span class="icon16 minia-icon-list-3"></span></label>').attr('for', $cb.attr('id') || ''));
